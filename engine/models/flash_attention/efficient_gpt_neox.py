...
class GPTNeoXAttention(nn.Module):
    def __init__(self, config):
        ...
        self.register_buffer(
            "norm_factor",
            torch.sqrt(torch.tensor(self.head_size, dtype=torch.float32)).to(self.dtype),
            persistent=False,
        )
    ...
    def forward(
        ...
        query = torch.cat((query, query_pass), dim=-1).to(query_rot.dtype)
        key = torch.cat((key, key_pass), dim=-1).to(key_rot.dtype)
        ...
        # Compute attention
        attn_output = self._attn(query.permute(0, 2, 1, 3), key.permute(0, 2, 1, 3), value.permute(0, 2, 1, 3), attention_mask, head_mask)
